...
The `sheets/ts/` workspace is green (`npm run typecheck`, `npm test` 6/6, Node running `.ts`
...
Every `.gs` file under [`../../sheets/`](../../sheets/) stays the deployed source of truth and is
not edited until the Phase E cutover. The **step list** below is the plan of record — ✅ done,
...
Facts worth keeping in view while reading the steps: there are **4 time-triggers**
(`syncTodoist`, `syncTodoistIntraday`, `syncEverhour`, `checkBillRisk`) and roughly **30 more
...
No TypeScript yet. This phase ends copy-paste deployment and unblocks the life-areas deploy.

| Step | What lands | Proof it's done |
| --- | --- | --- |
| **A1** ✅ | `sheets/ts/` workspace: `package.json`, `tsconfig.json`, `.gitignore`, `README.md`, and `src/config.ts` — the fail-closed settings reader, with tests. Root gains `workspaces`, `typecheck`/`test` delegation, a split `format:gs` / `format:ts`, and a committed lockfile | `npm run typecheck` clean, `npm test` 6/6 green — Node runs `.ts` with no build step, as designed |
...
| **A4** ⏳🔶 | Manifests **captured** to `sheets/ts/appsscript/{todoist,everhour}.json`. Remaining: [three decisions](#a4-the-three-open-decisions) | `git diff` reviewed by you, then committed by you |
| **A5** | A `prepare-legacy` script that stages each project's `.gs` set + its manifest into `dist/<project>/`, and the four `push:*` scripts (`sync`, `everhour`, `log`, `health`) | `npm -w sheets/ts run push:sync` round-trips: push, then a fresh pull matches |
...
| **A7** | `CLAUDE.md`, `README.md`, `sheets/README.md` updated: paste → `clasp push` | The setup steps no longer mention the editor |
...
All three standalone projects pulled into the scratchpad and diffed against the repo, with
...
| Finding | Detail |
| --- | --- |
| **There are two standalone projects, not one** | *Quantified Self - Todoist Sync* (10 files) and *Quantified Self - Everhour Sync* (1 file, still named `Código.js`). The docs describe a single `quantified-self-sync` project holding both; that has never been true. This **settles the "one project or two?" open question** — it is already two, and the port should keep them apart |
...
| 3 | **Do the manifests belong in `sheets/ts/appsscript/`?** | That is where they now sit, matching the target layout, and A5 stages them into `dist/<project>/`. The alternative is beside the `.gs` files, which cannot work — four projects would need four manifests in overlapping directories |

**Verified against clasp v3.4.1** (the earlier "check the flags" note is now settled):

| Finding | Consequence |
| --- | --- |
| `-P, --project <file>` selects a config | Three projects from one workspace works as planned: `.clasp-sync.json`, `.clasp-log.json`, `.clasp-health.json` |
| `srcDir` is the v3 name (`rootDir` still accepted), and clasp **refuses a `srcDir` that escapes the config's own directory** | A config in `sheets/ts/` *cannot* push from `sheets/todoist/`. Combined with each project needing its own `appsscript.json` — and `health-webhook.gs` sitting in `sheets/` alongside everything else — **pushing from the working tree is impossible without moving files.** Hence the staged `dist/<project>/` in A5, which also keeps clasp from ever writing into the working tree |
...
| **B4** | esbuild build → `dist/{sync,everhour,log,health}/`, manifest copied in, IIFE + generated global footer. Push scripts split into `push:staging` (default) and `push:prod` (refuses without `--confirm`) | A hello-world entry point pushes and runs in the **A8 staging project**, writing to the test copy |
...
| Step | What lands | Proof it's done |
| --- | --- | --- |
| **C1** | `core/dates.ts` — the duplicate `toDateString` dies here. One `formatUtcDay()`, one `formatDayInTimezone(date, timezone)`, never an ambient timezone | Tests for the UTC-vs-local evening case that [`history.md`](../../sheets/todoist/history.md) records as a real 2026-08-20 bug |
...
| **D10** | Everhour | Independent of all the above; the rate limiter becomes a service policy |
| **D11** | Reschedule habits | The only write path to Todoist. Ported late, deliberately, and stays manual |
| **D12** | The two webhooks | Tiny, but separate clasp projects with their own deploys |
...
| **E5** | Freeze `sheets/**/*.gs` — a header line on each file saying so |
| **E6** | `docs/todoist/architecture.md` rewritten once, to describe `sheets/ts/` |
...
| **F4** 🔶 | Workflow + secrets: `TODOIST_TOKEN`, `EVERHOUR_API_KEY`, the two spreadsheet IDs, `GOOGLE_SERVICE_ACCOUNT_JSON` |
...
| --- | --- | --- |
| Existing `.gs` | **Kept, untouched, deployed** | The port is a parallel tree, not a refactor. `sheets/**/*.gs` stays the running implementation *and* the thing the port is diffed against, right through Phase E |
| Port location | `sheets/ts/`, a self-contained **npm workspace** | Code sits beside the schema docs it must satisfy. Every Node/TS/clasp config lives inside the workspace, so the root stays free for a future `app/` Swift project |
...
| Write model | A `WritePlan` value with three verbs — `append`, `replaceWhere`, `upsert` | Replaces eight hand-rolled write strategies with three, and maps 1:1 onto the tab-strategy table in [`sheets/todoist/README.md`](../../sheets/todoist/README.md) — the invariant the port must not break |
...
| Tests | `node --test` over `sheets/ts/src/**/*.test.ts` | Zero new dependency on Node 24. Overrides the CLAUDE.md "no test framework unless asked" norm — asked for, because parity is the only proof the port is correct |
| Lockfile | **Now committed** | `npm ci` in Actions requires it, and there are real dependencies for the first time |
| Webhooks | Ported, but never leave Apps Script | Apple Shortcuts POST to a Web App URL. A scheduled Actions job has no inbound endpoint |
...
| `services/` | Per-source knowledge: Todoist/Everhour request descriptors, parsers, row mappers, the pagination rule | **No.** Describes requests, never issues them |
...
starts from identical data. `quantified-self-everhour` gets the same treatment when Phase D
reaches D10.

| Rail | What it is | What it catches |
| --- | --- | --- |
| **Separate Apps Script project** | The staging project from A8 is standalone, with its own Script Properties | Script Properties are **per project, shared by every file in it**. Push the port into `quantified-self-sync` and it inherits `TODOIST_SPREADSHEET_ID` = live, and the first run writes production. This is the likeliest way to get hurt, and a separate project is the only real defence |
...
Today ~34 callable functions are scattered across 11 files with no index. Instead, one array is
...
3. **`sheets/ts/ENTRYPOINTS.md`** is generated from it — a table of every entry point, its kind,
...
1. **One owner per document.** [`../todoist/architecture.md`](../todoist/architecture.md)
   describes *the deployed implementation* — the `.gs` files until Phase E, `sheets/ts/` after,
   rewritten once at E6. There is never a second architecture doc; until then it gains one line
   pointing here.
2. **`sheets/ts/README.md` is the human-readable architecture doc** — written for someone
...
- ✅ **~~One Apps Script project or two?~~ Settled by the A3 pull: it is already two.** Todoist
  ...
  keeps them apart, which also means `everhour-sync.gs`'s inlined query builder was never
  avoiding a real collision and can use the shared helper once ported.
