...
const EVERHOUR_API_KEY =
	PropertiesService.getScriptProperties().getProperty("EVERHOUR_API_KEY");
const EVERHOUR_SPREADSHEET_ID =
	PropertiesService.getScriptProperties().getProperty(
		"EVERHOUR_SPREADSHEET_ID",
	);
...
function syncEverhour() {
	...
	try {
		...
		errors.push(`TimeEntries: ${e.message || e}`);
		Logger.log(`TimeEntries FAILED: ${e}`);
	}

	try {
		rebuildDailySummary(7); // recalculate last 7 days from whatever's in the sheet
	} catch (e) {
		errors.push(`DailySummary: ${e.message || e}`);
		Logger.log(`DailySummary FAILED: ${e}`);
	}

	if (errors.length)
		throw new Error(
			"Some Everhour steps failed → " + errors.join(" | "),
		);
}
...
function fetchTimeEntries(daysBack) {
	...
	return entries.map((e) => {
		...
		return {
			...
			_norm: {
				...n,
				project_name: projectMap[n.project_id] || "",
			},
		};
	});
}
...
function rebuildDailySummary(rebuildDays) {
	...
	const cutoff = toDateString(
		new Date(Date.now() - rebuildDays * 86400000),
	);
	...
	for (let i = summaryData.length - 1; i >= 1; i--) {
		if (String(summaryData[i][0]) >= cutoff)
	...
	const rows = Object.entries(byDate)
		...
		.map(([date, d]) => {
			const totalHours =
				Math.round((d.total / 3600) * 100) / 100;
			const billableHours =
				Math.round((d.billable / 3600) * 100) / 100;
			const nonBillableHours =
				Math.round(
					((d.total - d.billable) / 3600) * 100,
				) / 100;
			const billablePct =
				d.total > 0
					? Math.round(
...
function everhourGet(path, params) {
	...
	if (code >= 300) {
		throw new Error(
			`Everhour GET ${path} → HTTP ${code}: ${body.slice(0, 200)}`,
		);
	}
	const trimmed = (body || "").trim();
	if (!trimmed.startsWith("{") && !trimmed.startsWith("[")) {
		throw new Error(
			`Everhour GET ${path} returned non-JSON: ${trimmed.slice(0, 200)}`,
		);
	}
...
function testEverhour() {
	if (!EVERHOUR_API_KEY)
		throw new Error(
			"Set EVERHOUR_API_KEY in Script Properties first.",
		);
	const shape = (r) =>
		Array.isArray(r)
			? `array[${r.length}]`
			: r && typeof r === "object"
				? `object keys: {${Object.keys(r).join(", ")}}`
				: String(r);
	const probe = (label, fn) => {
		try {
			Logger.log(`✓ ${label} → ${shape(fn())}`);
		} catch (e) {
			Logger.log(`✗ ${label} → ${e}`);
		}
	};
	...
	probe("GET /team/time", () =>
		everhourGet("/team/time", { from, to, limit: 1 }),
	);
}
...
function dumpEverhourSample() {
	...
	try {
		...
		Logger.log("projects sample failed: " + e);
	}

	try {
		const entries = everhourGet("/team/time", {
			from,
			to,
			limit: 1,
		});
		Logger.log(
			"Sample /team/time[0] (default):\n" +
				JSON.stringify(entries[0] || entries, null, 2),
		);
		const withBilling = everhourGet("/team/time", {
			from,
			to,
			limit: 1,
			opts_include_billing: 1,
		...
		Logger.log(
			"Sample /team/time[0] (with opts_include_billing=1):\n" +
				JSON.stringify(
					withBilling[0] || withBilling,
					null,
					2,
				),
		);
	} catch (e) {
		Logger.log("time-entries sample failed: " + e);
	}
}
