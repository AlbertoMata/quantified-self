...
Script webhook ([health-webhook.gs](health-webhook.gs)). No third-party app, no subscription.
...
> Build/setup of the shortcut itself: [../shortcuts/health-sync.md](../shortcuts/health-sync.md).
...
2. **Extensions → Apps Script** → paste [health-webhook.gs](health-webhook.gs) → Save.
...
4. Build the **Health Sync** shortcut and paste that URL into it — see
   [../shortcuts/health-sync.md](../shortcuts/health-sync.md). It runs daily at ~23:55 via a
...
- Adding a metric later = add a column here, add the matching key to the shortcut's Dictionary,
  and add the name to `COLS` in [health-webhook.gs](health-webhook.gs).
